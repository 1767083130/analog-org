...
const rateUtils = require('./services/rateUtils');
...
let position = new class{
    ...
    async getPositions(site,lazyMode = false){
        ...
        if(!getRateRes.isSuccess){
            return { isSuccess: false, message: "系统异常！获取比特币对美元汇率时发生错误"}
        }
        ...
        return { 
            ...
            data: {
                ...
                "coins": coins //各个币种的持仓情况 e.g [{"coin": "okb","total": 0.0000034144,"items": [{"site": "okex","total": 0.0000034144}] }]
            }
        }
    }
    ...
    async getBtcRate(){
        try{
            let getDepthRes = realTimePrice.getSymbolDepths(MarketSite,BtcSymbol);
            if(!getDepthRes.isSuccess){
                return { isSuccess: false };
            }
            // let getRateRes = await rateUtils.getUsdRate();
            // if(!getRateRes.isSuccess){
            //     return { isSuccess: false };
            // }

            let depth = getDepthRes.data;
            //let usdRate = getRateRes.rate;
