...
const debug = require('debug')('analog:lib:strategyPlan');
...
let strategyPlan = new class{
            ...
            for(let strategyItem of strategyPlanLog.strategys){
                ...
                if(strategy.relatedStrategy){ //设置了平仓策略的策略
                    ...
                    if(strategy.direction == 1){
                        ...
                        totalConsignAmount += Math.abs(consignAmount);
                        totalActualAmount += Math.abs(actualAmount);
                    ...
                    totalConsignAmount += Math.abs(strategyItem.consignAmount);
                    totalActualAmount += Math.abs(strategyItem.actualAmount);
                }
    ...
    async resetStrategyPlan(strategyPlan,status){
        ...
        let planStrategys = await TransferStrategy.find({ _id: {$in: strategyIds},isValid: true }).sort({ created: 1,direction: 1 });

        let newStrategyIds = [],
            newStrategys = [], strategyPairs = []; //e.g [{ oldId: '234234',newId: '24324' }]
        for(let i = 0; i < planStrategys.length; i++ ){
            ...
            //transferStrategy.relatedStrategy = 
            let newStrategy = await transferStrategy.save();
            newStrategys.push(newStrategy);
            newStrategyIds.push({ strategyId: newStrategy._id});
            strategyPairs.push({ oldId: oldStrategy._id, newId: newStrategy._id });
        }

        for(let strategyItem of newStrategys){
            if(strategyItem.relatedStrategy){
                let pairItem = strategyPairs.find(p => p.oldId.toString() == strategyItem.relatedStrategy.toString());
                strategyItem.relatedStrategy = pairItem.newId;
                await strategyItem.save();
            }
        }
    ...
    async updateStrategyPlanAmount(strategyPlanLog,options){
        await BaseStrategyRunner.updateStrategyPlanAmount(strategyPlanLog,options);
    }

    /**
     * 判断是否能获取到策略计划所需要的数据类型
     * @param {Object} strategyPlanLog 需要运行的计划实例
     */
    ...
    async getUnEndedPlanOrdersCount(strategyPlan){
