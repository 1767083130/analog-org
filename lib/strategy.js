...
let strategy = new class{
     ...
     * @param {StrategyPlanLog} [options.strategyPlanLog]  交易策略所属的策略Log
     ...
     * @returns 返回计算后的结果
     */
    async getExpressionValue(condition,strategyType,envOptions) { 
        let instance = BaseStrategyRunner.getInstance(strategyType);

        let strategyValue;
        if(instance.getExpressionValue){
            strategyValue = await instance.getExpressionValue(condition,envOptions);
        }
