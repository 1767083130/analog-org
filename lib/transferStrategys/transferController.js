...
let transferController = new class {
     ...
     * 
     */
    async renewApiOrder(apiOrder,order){
        ...
        if(apiOrder.status == 'canceled'  && order.waitCreateChild //如果是修改价格未完成的委托
           && order.apiStatus !== 'canceled' ){ //排除已经处理过的，避免重复处理
            ...
            if(parentOrder){
                await orderLib.createChildOrder(parentOrder);
            }
        }
        ...
        //如果委托满足这些条件，说明数据已经过期了。网络数据传输两条单独的数据（并非只的是数据包）不能保证先发送先到达，这里的处理非常必要
        if((Math.abs(realDealAmount) < Math.abs(order.bargainAmount))
            //|| (order.status == 'canceled' && apiOrder.status != 'canceled') //注释此行代码。考虑到委托被取消后，还有可能有成交数据传送过来（网络数据传输不能保证先发送先到达）
            || order.status == 'success'){
            debug('数据过期，不进行任何处理');
            return;
        }

        //对委托成交金额的变更和委托单状态变更进行处理
        ...
        if(transferStrategyLog.strategyPlanLogId){
            if(order.operateId == 1){ //必须是策略的第一步才更改计划相关的金额
                ...
                if(strategyPlanLog){
                    //委托结束的时候，纠正计划已委托数量
                    if(['canceled','success','failed'].indexOf(apiOrder.status) != -1  //订单已经结束
                        ...
                        let planOptions = {
                            ...
                            consignAmountChange: -Math.abs(realUnDealAmount),
                            log: {
                                source: '_3',
                                strategyId: transferStrategy._id,
                                consignAmountChange: -Math.abs(realUnDealAmount),
