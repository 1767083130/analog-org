...
let transferController = new class {
    ...
    async renewApiOrder(apiOrder){
        let order = await Order.findOne({ outerId: apiOrder.outerId,site: apiOrder.site });
        if( !order
           || (order && (!order.isSysAuto || order.reason != 'transfer' || !order.actionId))){
            return;
        }
        ...
        newOrder.changeLogs.push(JSON.stringify(apiOrder));
        ...
        if(transferStrategyLog.strategyPlanLogId){
            if(order.operateId == 1){ //必须是策略的第一步才更改计划相关的金额
                ...
                if(strategyPlanLog){
                    ...
                    //     planOptions.consignAmountChange = Math.abs(apiOrder.amount);
                    // }
                ...
                if(order.operateId == maxOperateId){
                    ...
                    if(strategyPlanLog){
                        ...
                        console.log(`actualAmountChange:${planOptions.actualAmountChange}`);
                        await BaseStrategyRunner.updateStrategyPlanAmount(strategyPlanLog,planOptions);
                    }
                }
            }
