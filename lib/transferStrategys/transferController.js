...
let transferController = new class {
    ...
    async onOrderStatusChanged(e){
        ...
        let strategyPlanLog;
        if(transferStrategyLog.strategyPlanLogId && e.order.operateId == 1){ //必须是策略的第一步才更改计划相关的金额
                ...
                if(run){
                    try {
                        ...
                        if(Math.abs(e.stepAmount) > 0){
                            ...
                            let stepAmount = (nextOperate.orderAmount > 0 ? Math.abs(e.stepAmount) : -Math.abs(e.stepAmount)) * times;
                            let options = {
