...
class BaseStrategyRunner {
     ...
     * @param {Object} [options.strategyPlan] 执行策略的计划。如果为空，则表示直接执行单独的交易策略，否则就是执行计划中的策略
    ...
    async runStrategy(strategy,options){
        ...
        let getLogRes = await this.getTransferStrategyLog(conditionOrders,strategy,false,options.strategyPlan);
        ...
        if(!operateLog.orgOperate.previousOperate || (operateLog.orgOperate.previousOperate <= 0)) {
            if(operateLog.totalAmount != 0){
                ...
                if(!res.isSuccess){
                    ...
                    if(options.strategyPlan){
                        let planOptions = {
                            strategyId: strategy._id,
                            consignAmount: Math.abs(operateLog.totalAmount)
                        }
                        await BaseStrategyRunner.updateStrategyPlanAmount(options.strategyPlan,planOptions);
                    }
                }
            }
        }
    ...
    async getTransferStrategyLog(conditionOrders,strategy, checkAccount = false,strategyPlan = null) { 
        ...
        if(strategyPlan){
            transferStrategyLog.strategyPlanId = strategyPlan._id;
        }
        ...
        for(var i = 0; i < strategy.operates.length; i++){
            ...
            let amount = await this.getOperateAmount(operate.orderAmount,conditionOrders,envOptions);
            ...
            if(operate.action == 'transfer'){ //操作类型。分为 trade(成交)、transfer（转币）两种
                ...
                if(isFirstOperate && (Math.abs(operate.minOrderAmount) > Math.abs(totalAmount))){
                    res.isSuccess = false;
                    res.message = `在id为${operate.id}操作上，最多委托数量为${amount},但要求每单的最小量为${operate.minOrderAmount}`;
                    return res;
                }
    ...
    async getOperateAmount(amountExpression,conditionOrders,envOptions){
    ...
    /**
     * 更改策略计划中的金额
     * @param {*} strategyPlan 策略计划
     * @param {*} options 需要变更的金额
     * @param {*} options.strategyId 策略_Id
     * @param {Number} [options.actualAmount] 实际已成交数量的变更额度，比如2,表示成交2个对应的交易品种
     * @param {Number} [options.consignAmount] 实际已委托数量的变更额度，比如2,表示委托2个对应的交易品种，但有可能尚未成交（也有可能已成交）
     */
    static async updateStrategyPlanAmount(strategyPlan,options){
        if(!options.strategyId){
            throw new Error(`参数options.strategyId不能为空`);
        }

        strategyPlan.strategys = strategyPlan.strategys || [];
        let planStrategy = strategyPlan.strategys.find(p => p.strategyId.toString() == options.strategyId.toString());
        if(planStrategy){
            if(options.actualAmount){
                planStrategy.actualAmount = new Decimal(planStrategy.actualAmount).plus(options.actualAmount).toNumber();
            }
            if(options.consignAmount){
                planStrategy.consignAmount = new Decimal(planStrategy.consignAmount).plus(options.consignAmount).toNumber();
            }
        } else {
            planStrategy = {
                actualAmount: options.actualAmount,
                consignAmount: options.consignAmount,
                strategyId: options.strategyId
            };
            strategyPlan.strategys.push(planStrategy);
        }
     ...
     *     
     */
    async getStackItemValue(stackItem,conditionDepths,envOptions){ // [{ site: "huobi", symbol: 34,price: 324,amount: 34 }]
        var item = {}, //处理后的值
            itemValue,
            expressionType;
        ...
        if(expressionType == 'variable'){
            ...
            if(assignPriceInfo){  //如果表达式表示的是符合条件的委托价格 如 huobi.btc.buy或huobi.btc.sell
                ...
                let variableValue = await expression.getVariableValue(stackItem,envOptions);
