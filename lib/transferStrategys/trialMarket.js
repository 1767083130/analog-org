...
let trialMarket = new class{
    ...
    /**
     * 试算各个网站的交易品种的差价
     * @param {*} siteItems e.g [{ site: "bitfinex", contractType: "spot"},{site: "okex", contractType: "futures"}];
     */
    async trialNoLimit(siteItems){
        let resItems = [];
        for(let i = 0; i < siteItems.length; i++){
            let siteItemA = siteItems[i];
            for(j = i + 1; j < siteItems.length; j++){
                let siteItemB = siteItems[j];
                let item = await this.trialPairNoLimit(siteItemA,siteItemB);
                resItems.push(item);
            }
        }
     ...
     */
    async trialPairNoLimit(siteItemA,siteItemB,realPrices,includeUnvalid){
        ...
        */
        ...
        if(!realPrices){
            realPrices = [];
            let all_siteA_realPrices = realTimePrice.getSymbolDepths(siteItemA.site);
            let all_siteB_realPrices = realTimePrice.getSymbolDepths(siteItemB.site);
        ...
        let symbolPathPairsRes = trialMarketSymbol.getSymbolPathPairs(siteItemA.site,siteItemB.site,realPrices);
