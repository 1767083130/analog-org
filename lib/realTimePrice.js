...
let realTimePrice = new class{
     ...
     * @param {Number} [expired] 行情数据有效期。默认为0.5分钟
     * @returns {Object} e.g { isSuccess: true, data: { site: "huobi",timestamp: +now(), symbol: "btc#usd",bids:[],asks:[] }  }
     *          其中，当symbol为空时，data为数组;不为空时，data为Object类型
     */
    getSymbolDepths(site,symbol,expired = 0.5 * 60 * 1000){
        let getDepthsRes = cacheClient.getSymbolDepths(site,symbol);
        if(getDepthsRes.isSuccess){
            if(+Date.now() - getDepthsRes.data.timestamp > expired){
                return { isSuccess: false, expired: true, message: "行情数据已超过有效期" }
            }
        }
        return getDepthsRes;
    }
    ...
    getSymbolPrice(site,symbol, maxStepsCount = 1, getDepthPrice){
        ...
        if(sameSymbol){
            let getDepthsRes = this.getSymbolDepths(site,symbol);
            if(getDepthsRes.isSuccess){
                return {
                    isSuccess: true,
        ...
        let getDepthsRes = this.getSymbolDepths(site);
        if(!getDepthsRes.isSuccess){
        ...
        for(let symbolPath of symbolPaths){
            ...
            let getSymbolDepthRes = this.getSymbolDepths(site,symbolPath.symbol);
