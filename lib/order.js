...
const bothApiLib = require('./apiClient/bothApi');
//const strategyPlanLib = require('./strategyPlan');
const BaseStrategyRunner = require('./transferStrategys/baseStrategyRunner');
...
const debug = require('debug')('analog:lib:order');
...
class OrderController extends EventEmitter{
    ...
    async runOrderOperate(logOperate,options){
        ...
        let order = {
            ...
            type: operate.type, //exchage/margin
            ...
            strategyPlanLogId: transferStrategyLog.strategyPlanLogId,
            isSysAuto: true,
        ...
        res = await this.createOrder(order,identifier,false,options);
        if(res.isSuccess){
     ...
     * @public
     */
    async createOrder(order,identifier,refreshAccount = false,options){
        if(!identifier){
        ...
        let bothApi = bothApiLib.getInstance(identifier);
        let newOrder = new Order(order);
        newOrder.modified = new Date();
        newOrder.apiOrder = options && options.apiOrder;
     ...
     * @param {*} order 委托单
     * @param {*} options 
     * @param {Number} [options.minStepsCount] 默认为1，当前设置的委托价格至少距离市场深度下探到minStepsCount个层级时，才修改价格.当IsPostOnly=true时有效
     * @param {Number} [options.minAmount] 默认为0，当前设置的委托价格至少距离市场深度下探到minAmount委托量时，才修改价格.当IsPostOnly=true时有效
    ...
    async replaceOrder(order, options) {
        ...
        if(replaceOrderSupported){ //如果api直接支持replaceOrder接口
            ...
            if(!apiRes.isSuccess){
                return { isSuccess: false, errorCode: "100005", message: `交易平台返回错误信息：${apiRes.message}` };
            } 
            ...
            if(order.outerId == apiRes.outerId){ //如果只是修改委托
                ...
                let orderOptions = {
                    ...
                    type: order.type, //margin or exchange
                    ...
                    strategyPlanLogId: order.strategyPlanLogId,
     ...
     * @public
     */
    async cancelOrder(order,identifier){
        if(!order){    
     ...
     * @public
     */
    async retryOrder(order,newOrderOptions,identifier){
        //let api = new Api(identifier);
        if(!identifier){
            identifier = await clientIdentifier.getUserClient(order.userName,order.site);  
            if(!identifier){
        ...
        if(cancelRes.isSuccess){
            // //重新获取委托信息，尽可能避免脏数据带来的仓位问题
            // order = await Order.findOne({ outerId: order.outerId});
            // if(!order){
            //     return { isSuccess: false,message: `找不到第三方交易网站${order.site}id为${order.outerId}的委托,有可能已被删除` }
            // }
            // let undealAmount = order.amount - order.bargainAmount;
            // if(order.status == 'success' || Math.abs(undealAmount) < 0){
            //     return  {isSuccess: true,message: '已不需要进行重新委托' };
            // }

            //更改本地的委托
            order.status = "auto_retry";
            order.modified = Date.now();
            order.waitCreateChild = true;
            order.waitRetryPrice = newOrderOptions.price;
            //order.childOrder = newOrder._id; 
            order = await order.save();
        }
        return cancelRes;
    ...
    async createChildOrder(order,identifier){
        if(!identifier){
            identifier = await clientIdentifier.getUserClient(order.userName,order.site);  
            if(!identifier){
                return { isSuccess: false, message: `找不到授权信息。userName: ${order.userName},site: ${order.site}`};             
        ...
        var newOrder = new Order({
            ...
            price: order.waitRetryPrice, //委托价格
            orgPrice: order.orgPrice,
            amount: undealAmount,//newOrderOptions.amount, //总数量
        ...
        let strategyPlanLog = await StrategyPlanLog.findOne({ _id: order.strategyPlanLogId });
        if(strategyPlanLog && order.actionId && newOrder.operateId == 1){
            let strategyLog = await TransferStrategyLog.findOne({ _id: order.actionId });
            let planOptions = {
                strategyId: strategyLog.strategyId,
                consignAmountChange: undealAmount,
                log: {
                    source: '_8',
                    strategyId: strategyLog.strategyId,
                    consignAmountChange: undealAmount,
            ...
            debug('updateStrategyPlanAmount_8')
        }
    ...
    async syncOrders(options,stepCallBack) {
        ...
        //todo 这里要注意系统的健壮性
        //获取未成功并且未取消的委托  
        orders = await Order.findOne(filter).sort({ userName: -1}); //todo 如果用户量大或者多服务器时，可以考虑改善这里的做法
    ...
    async syncOrder(newOrder,oldOrder,identifier){
        if(!oldOrder){
            oldOrder = await Order.findOne({ outerId: oldOrder.id })
        }
        ...
        if(changeAmount > 0){
            let refreshOrderRes = await this.refreshOrder(oldOrder,newOrder,identifier);
     ...
     * @returns e.g { expired: true,order: newOrder  }
     * @private
     */
    async refreshApiOrder(apiOrder){
        ...
        if(apiOrder.status == 'canceled'  && order.waitCreateChild //如果是修改价格未完成的委托
           && order.apiStatus !== 'canceled' ){ //排除已经处理过的，避免重复处理
            ...
            let parentOrder = await Order.findOneAndUpdate({
                outerId: apiOrder.outerId,
                site: apiOrder.site 
            },{
                $set: { 
            ...
            if(parentOrder){
                await this.createChildOrder(parentOrder);
            }
        }
