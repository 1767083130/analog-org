    ...
    $.formValidator =
{
        ...
        if (initConfig.mode == "SingleTip") {
            ...
            if (html.length = 0) {
                ...
                if (elem.validatorPasswordIndex > 0 && showclass == "onCorrect") {
                    ...
                    //margin: 0px; padding: 0px; background: none repeat scroll 0% 0% transparent;
            ...
            if (stype == "password" || stype == "text" || stype == "file") {
                ...
                if (onFocusClass != "" && showclass == "onFocus") {
                    ...
                    if (onFormValidatorFocus) {
                        onFormValidatorFocus(jqobj);
                    }
                };
                if (onCorrectClass != "" && showclass == "onCorrect") {
                    jqobj.removeClass().addClass(onCorrectClass);
                    if (onFormValidatorCorrect) {
                        onFormValidatorCorrect(jqobj);
                    }
                };
                if (onErrorClass != "" && showclass == "onError") {
                    jqobj.removeClass().addClass(onErrorClass);
                    if (onFormValidatorError) {
                        onFormValidatorError(jqobj);
                    }
                };
            }
        }
    ...
    ajaxValid: function (e) {
        ...
        //updated by webnuke 支持setting.preAjax
        var params = [];
        if (setting.preAjax) {
            params = setting.preAjax() || [];
        }
        ...
        if (setting.type == "POST") {
            ...
            for (var i = 0; i < params.length; i++) {
                ls_data += "&" + params[i].name + "=" + params[i].value;
            }
        } else {
            ls_url = ls_url + (ls_url.indexOf("?") > -1 ? ("&" + parm) : ("?" + parm));
            for (var i = 0; i < params.length; i++) {
                ls_url += "&" + params[i].name + "=" + params[i].value;
            }
        }
        //endupdated 
...
var regexEnum =
{
    ...
    zipcode: "^\\d{6}$",						//邮编
    mobile: "^[1-9]\\d{10}$",   //"^13[0-9]{9}|15[012356789][0-9]{8}|18[0256789][0-9]{8}|147[0-9]{8}$",				//手机
