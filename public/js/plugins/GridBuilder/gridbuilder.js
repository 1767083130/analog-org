(function() {
    ...
    p = '<button type="button" name="btnEditId">id: none</button>',
    ...
    YAHOO.util.GridBuilder = {
        init: function() {
            ...
            this.docType = "doc";
            this.rows = [];
            this.rows[0] = f.get(this.prefixID + "ddlSplitBody0");
            ...
            var k = new YAHOO.widget.Button(this.prefixID + "btnSaveSkin");
...
//            b.on(this.bd, "mouseover", YAHOO.util.GridBuilder.mouseOver, YAHOO.util.GridBuilder, true);
            this.template = f.get(this.prefixID + "ddlWhichGrid");
            b.on(this.template, "change", YAHOO.util.GridBuilder.changeType, YAHOO.util.GridBuilder, true);
            b.on(this.prefixID + "ddlSplitBody0", "change", YAHOO.util.GridBuilder.splitBody, YAHOO.util.GridBuilder, true);
            b.on(this.prefixID + "ddlWhichDoc", "change", YAHOO.util.GridBuilder.changeDoc, YAHOO.util.GridBuilder, true);
            b.on(this.bd, "mouseover", YAHOO.util.GridBuilder.mouseOver, YAHOO.util.GridBuilder, true);
            ...
            var k = new YAHOO.widget.Button(this.prefixID + "btnShowCode");
            k.on("click", YAHOO.util.GridBuilder.showCode, YAHOO.util.GridBuilder, true);
            var j = new YAHOO.widget.Button(this.prefixID + "btnResetBuilder");
            j.on("click", YAHOO.util.GridBuilder.reset, YAHOO.util.GridBuilder, true);
            var i = new YAHOO.widget.Button(this.prefixID + "btnAddRow");
            i.on("click", YAHOO.util.GridBuilder.addRow, YAHOO.util.GridBuilder, true);
            var n = new YAHOO.widget.Button(this.prefixID + "btnDocReturn");
            n.on("click",
            function(o) {
        ...
        setId:function(k){
            ...
            //j = l.cfg.getProperty("text"),
            i = f.getAncestorByTagName(this.get("element"), "div");
            var oldValue = i.getAttribute("id");
            var id = oldValue;
            var isFirst = true;
            
            if(oldValue.toLowerCase() != "contentpane"){
                do{
                   if(!isFirst){
                       if(!id){
                           alert("id不能为空。");
                           ...
                           alert("已存在id为" + id + "的元素，请重新输入。");
                       }
                   }
                   
                   //todo 使用trim去掉空格
                   var defaultValue = oldValue;
                   if(defaultValue == ""){
                       var i = 0;
                       while(document.getElementById("ContentPane" + i)){
                           i++;
                       }
                       defaultValue = "ContentPane" + i;
                   }
                   
                   id = prompt("请输入ID",defaultValue);
                   if(id == null){ //按取消键
                       id = oldValue;
                       break;
                   }
                   ...
                   isFirst = false;
                }while(!id && id != oldValue && !document.getElementById(id))
            }
            else{
                alert("必须保证一个面板的ID为contentpane，<br/>将先将另一个面板的ID更换为contentpane后，再重新设置。");
            }
            
            //如果将面板id更换为contentpane，则更改原先的contentpane面板
//            if(id.toLower() == "contentpane" && oldValue.toLower() != "contentpane"){
//                var contentPane = document.getElementById("ContentPane");
//                var i = 0;
//                while(document.getElementById("ContentPane" + i)){
//                    i++;
//                }
//                var newId = "ContentPane" + i;
//                contentPane.setAttribute("id",newId);
//            }
            if(id.toLowerCase() == "contentpane" && oldValue.toLowerCase() != "contentpane"){
                var contentPane = document.getElementById("ContentPane");
                var i = 0;
                while(document.getElementById("ContentPane" + i)){
                    i++;
...
//            }
        ...
        createIdButton:function(){
            var oPushButton = new YAHOO.widget.Button("pushbutton1");   //"pushbutton1"
            //oPushButton.on("click", this.setId);  
            oPushButton.on("click", function(){});
            return oPushButton; 
...
//            return o
        ...
        reset: function(k) {
            ...
            this.rows = [];
            this.rows[0] = f.get(this.prefixID + "ddlSplitBody0");
            f.get(this.prefixID + "ddlWhichDoc").options.selectedIndex = 0;
            f.get(this.prefixID + "ddlWhichGrid").options.selectedIndex = 0;
            f.get(this.prefixID + "ddlSplitBody0").options.selectedIndex = 0;
        ...
        addRow: function(l) {
            ...
            var k = f.get(this.prefixID + "ddlSplitBody0").cloneNode(true);
            ...
            var i = f.get(this.prefixID + "ddlSplitBody0").parentNode.parentNode.parentNode;
            i.insertBefore(j, f.get(this.prefixID + "btnAddRow"));
        ...
        delRow: function(j) {
            ...
            f.get(this.prefixID + "ddlSplitBody0").parentNode.parentNode.parentNode.removeChild(i.parentNode);
            this.rows[k] = false;
            this.splitBody();
            if (this.useARIA) {
                this.restoreLandmarksState()
        ...
        changeDoc: function(i) {
            ...
            this.currentDocType = f.get(this.prefixID + "ddlWhichDoc").selectedIndex;
            this.docType = f.get(this.prefixID + "ddlWhichDoc").options[this.currentDocType].value;
        ...
        splitBody: function() {
            ...
            for (var j = 0; j < this.rows.length; j++) {
                this.splitBodyTemplate(f.get(this.prefixID +"ddlSplitBody" + j))
            }
            if (!this.storeCode) {
                this.doTemplate()
            }
        ...
        splitBodyTemplate: function(i) {
            var isFirstRow = (i.id == this.prefixID + "ddlSplitBody0");
            if (i) {
        ...
        showSlider: function() {
            ...
            var o = function() {
                f.get(this.prefixID + "ddlWhichDoc").selectedIndex = this.prevDocType;
                this.changeDoc();
                n.hide()
            };
            var k = function(r) {
